...
start-backend:
	...
	@poetry run opentelemetry-instrument uvicorn openhands.server.listen:app --host $(BACKEND_HOST) --port $(BACKEND_PORT) --reload --reload-exclude "./workspace"
...
_run_setup:
	...
	@poetry run opentelemetry-instrument uvicorn openhands.server.listen:app --host $(BACKEND_HOST) --port $(BACKEND_PORT) &
